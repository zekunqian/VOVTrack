...
class StandardRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        # removed loading complete and incomplete prompt
        ...
        # self.special_prompt = self.special_prompt / self.special_prompt.norm(dim=1, keepdim=True)
        # self.special_prompt = torch.nn.functional.normalize(self.special_prompt, p=2, dim=1)

        # adding self customized prompt groups.
        self.prompt_group_list = prompt_group_list
        self.prompt_group_mean_way = prompt_group_mean_way
        self.prompt_prob_level_calc = prompt_prob_level_calc
        self.vis = vis
        ...
        if self.prompt_group_list is not None:
            ...
            self.special_prompt = torch.nn.functional.normalize(self.special_prompt, p=2, dim=1).view(-1, 2, 512).permute(0, 2, 1)
        ...
        self.use_part_layer_to_extract_features = use_part_layer_to_extract_features
        self.prompt_prob_train = prompt_prob_train
        self.prompt_prob_test = prompt_prob_test
        self.train_kd_loss = train_kd_loss
        self.softmax_mode=softmax_mode
        self.low_level_num = 0
        self.high_level_num = 0
        self.total_num = 0
    ...
    def _bbox_forward(self, x, rois):
        ...
        rois = rois.float()
        if self.use_part_layer_to_extract_features:
            # only use pre layer to detection
            bbox_feats = self.bbox_roi_extractor(
                x[:self.bbox_roi_extractor.num_inputs], rois, det_cls_layer_tag=0)
            if self.with_shared_head:
            ...
            # only use tail layer to clssification
            cls_feats = self.bbox_roi_extractor(
                x[:self.bbox_roi_extractor.num_inputs], rois, det_cls_layer_tag=1)
            if self.with_shared_head:
                cls_feats = self.shared_head(cls_feats)
            region_embeddings_cls = self.bbox_head.forward_embedding(cls_feats)
            return bbox_results, region_embeddings_cls
        else:
            bbox_feats = self.bbox_roi_extractor(
    ...
    def _bbox_forward_train(self, x, img, img_no_normalize, sampling_results, proposals_pre_computed, gt_bboxes, gt_labels,
            ...
            if self.load_feature:
                ...
                for i in range(len(img_metas)):
                    ...
                    try:
                        ...
                        stream = io.BytesIO(f)
                        tmp = torch.load(stream, map_location=self.device)
        ...
        # evaluation
        self.iters += 1
        if self.iters<1: # orginal one is 200
        ...
        # img_complete_result = clip_image_features_ensemble @ self.special_prompt.float().T
        # img_complete_result = img_complete_result.softmax(dim=-1)
        # img_condition = img_complete_result[:, 0] > img_complete_result[:, 1]
        # img_selected_indices = torch.nonzero(img_condition).squeeze()

        # text_complete_result = region_embeddings@ self.special_prompt.float().T
        # text_condition = text_complete_result[:, 0] > text_complete_result[:, 1]
        # text_selected_indices = torch.nonzero(text_condition).squeeze()
        ...
        if self.prompt_prob_train:
            if self.prompt_group_list is None:
                with torch.no_grad():
                    text_complete_result = region_embeddings @ self.special_prompt.float().T
                ...
                probs = region_embeddings@ self.special_prompt.float()
                probs = probs / 0.007
                probs = probs.softmax(dim=-1).permute(1, 0, 2)
                # statistic code: probs[:,:,0].permute(1,0).mean(dim=1) # used to statistic mean prob of all adj prompts
                if self.prompt_group_mean_way:
                    probs = probs.mean(dim=1)[:, 0].view(-1, 1)  # [:, 0].view(-1, 1)
                if self.vis:
                    # prepare the probs and the index of the candidate
                    # image_tensor = img_no_normalize.squeeze(0)
                    # image_tensor = (image_tensor - image_tensor.min()) / (image_tensor.max() - image_tensor.min())
                    img_name = os.path.split(img_metas[0]['filename'])[1]
                    # save_image(image_tensor, os.path.join(self.save_vis_dir, img_name))
                    end_index = torch.nonzero(torch.eq(labels, 1203), as_tuple=False)[0]
                    indices = []
                    prob_selected_list = []
                    for i in range(len(probs)):
                        if probs[i] >= self.vis_interval[0] and probs[i] <= self.vis_interval[1] and i < end_index:
                            indices.append(i)
                            prob_selected_list.append(round(probs[i].item(), 3))
                    # getting the bbox of the candidate
                    pos_bbox = sampling_results[0].pos_bboxes[indices]
                    pos_bbox_lables = sampling_results[0].pos_gt_labels[indices]
                    mask = ~torch.any(pos_bbox_lables.unsqueeze(1) == self.novel_label_ids, dim=1)
                    pos_bbox = pos_bbox[mask]
                    boxes = pos_bbox.cpu()
                    if len(indices) > 0:
                        # NMS
                        nms_scores = torch.tensor(prob_selected_list)
                        nms_threshold = 0.5
                        keep_index = nms(boxes, nms_scores, nms_threshold)
                        boxes = boxes[keep_index]
                        prob_selected_list = [prob_selected_list[index] for index in keep_index]
                        ...
                        # Define the rectangle tensor; each row: x1, y1, x2, y2


                        for i, box in enumerate(boxes):
                            x1, y1, x2, y2 = box
                            draw.rectangle([x1, y1, x2, y2], outline='red')
                            draw.text((x1, y1), str(prob_selected_list[i]), fill='red', font=font)

                        image_pil.save(os.path.join(self.save_vis_dir, img_name))

                if self.prompt_prob_level_calc:
                    # level
                    probs[probs <= self.level_low] = 0
                    probs[probs >= self.level_high] = 1

                    if self.debug:
                        self.low_level_num += torch.sum(probs <= self.level_low).item()
                        self.high_level_num += torch.sum(probs >= self.level_high).item()
                        self.total_num += probs.shape[0]
                        if self.total_num % 1000 == 0:
                            print(f"low: {round(self.low_level_num/self.total_num, 2)}, "
                                  f"high: {round(self.high_level_num/self.total_num, 2)}, "
                                  f" low num: {self.low_level_num}, high num: {self.high_level_num}, total: {self.total_num}")
            ...
            if self.softmax_mode:
                cls_score_text = cls_score_text*complete_prob

        # self.iters += 1
        cls_score_text[:,self.novel_label_ids] = -1e11
        if self.prompt_prob_train and self.softmax_mode is False:
            if self.only_person:
                complete_prob = (labels == 792).unsqueeze(1) * complete_prob
            text_cls_loss = self.row_weight_cross_entropy(cls_score_text / self.temperature, labels, complete_prob)
        else:
            text_cls_loss = F.cross_entropy(cls_score_text / self.temperature, labels, reduction='mean')
        # kd_loss = F.l1_loss(region_embeddings_image[img_selected_indices],clip_image_features_ensemble[img_selected_indices])
        if self.train_kd_loss:
        ...
        if self.train_kd_loss:
            loss_bbox.update(kd_loss=kd_loss * self.kd_weight)
    ...
    def row_weight_cross_entropy(self, input, target, prob_weight):
        """
        :param input: input tensor without softmax
        :param target: [0,1,3] ... label
        :param prob_weight: torch.tensor([0.3,0.7,0.6,0.9]) prob_weight = torch.tensor([0.1, 0.3, 0.6]).view(-1, 1)
        :return:
        """
        # Compute log probabilities of model outputs
        log_probs = F.log_softmax(input, dim=1)

        # Gather the log probability of the true class for each sample
        target_log_probs = log_probs.gather(1, target.view(-1, 1))
        if len(prob_weight.shape) < 2:
            prob_weight = prob_weight.view(-1, 1)
        target_log_probs = target_log_probs*prob_weight
